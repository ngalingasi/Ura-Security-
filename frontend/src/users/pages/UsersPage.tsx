...
// ── Modal — NO backdrop-blur, proper scroll ───────────────────────────────────
...
function Modal({ isOpen, onClose, title, children, size = 'md' }: {
  ...
  return createPortal(
    // Outer — scrollable, NO backdrop-blur (kills GPU perf)
    <div className="fixed inset-0 z-[99999] flex items-start justify-center overflow-y-auto"
      style={{ paddingTop: '5vh', paddingBottom: '5vh' }}
      onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="fixed inset-0 bg-black/40 dark:bg-black/60" />
      <div className={`relative w-full ${MODAL_SIZE[size]} mx-4 bg-white dark:bg-gray-900
        rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col`}
        style={{ maxHeight: '90vh' }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          <h3 className="text-base font-semibold text-gray-800 dark:text-white">{title}</h3>
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800">
            <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>
        </div>
        {/* overscroll-contain stops scroll leaking to body */}
...
// ── Form field helpers — NO transition-colors (causes input cursor delay) ─────
const inputBase = [
  ...
  // intentionally NO transition-colors
].join(' ');

function FInput({ label, required, error, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string; error?: string; required?: boolean }) {
  return (
    <div>
      ...
      <input className={`${inputBase} ${error ? 'border-red-400' : ''}`} {...props} />
      {error && <p className="mt-1 text-xs text-red-500">{error}</p>}
    </div>
  );
}
function FSelect({ label, required, error, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; error?: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">{label}{required && <span className="text-red-500"> *</span>}</label>
      <select className={`${inputBase} ${error ? 'border-red-400' : ''}`} {...props}>{children}</select>
      {error && <p className="mt-1 text-xs text-red-500">{error}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">{title}</p>
...
function Avatar({ name }: { name: string }) {
  ...
  return (
    <div className={`w-10 h-10 ${colors[name.charCodeAt(0) % colors.length]} rounded-full flex items-center justify-center text-white text-sm font-semibold flex-shrink-0`}>
      {initials}
    </div>
  );
}
...
function UserForm({ user, allSkills, isSuperAdmin: superAdmin, onSaved, onClose }: {
  ...
  const [form, setForm] = useState({
    ...
    gender: user?.gender ?? 'male', role: user?.role ?? 'user',
  ...
  return (
    <form onSubmit={save} className="space-y-5">
      ...
      <FSection title="Personal Information">
        <FInput label="Full Name" required value={form.full_name} onChange={e => set('full_name', e.target.value)} placeholder="e.g. John Doe" />
        <div className="grid grid-cols-2 gap-3">
          <FInput label="Username" required value={form.username} onChange={e => set('username', e.target.value)} placeholder="johndoe" disabled={isEdit} />
          <FInput label="Email" type="email" value={form.email ?? ''} onChange={e => set('email', e.target.value)} placeholder="john@example.com" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <FInput label="Mobile" value={form.mobile ?? ''} onChange={e => set('mobile', e.target.value)} placeholder="+255712345678" />
          <FSelect label="Gender" value={form.gender ?? 'male'} onChange={e => set('gender', e.target.value)}>
            <option value="male">Male</option>
            <option value="female">Female</option>
          </FSelect>
        </div>
      </FSection>
      <FSection title="Role & Access">
        <div className="grid grid-cols-2 gap-3">
          <FSelect label="Role" required value={form.role} onChange={e => set('role', e.target.value)}>
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={saving} className="px-5 py-2 text-sm font-medium bg-brand-500 text-white rounded-lg hover:bg-brand-600 disabled:opacity-50 flex items-center gap-2">
              ...
              {loading
                ...
                : data.results.map(u => (
                    <tr key={u.user_id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                          ...
                          <div>
                            ...
                            {u.skills && u.skills.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                ...
                                {u.skills.length > 3 && <span className="text-[10px] text-gray-400">+{u.skills.length - 3}</span>}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full capitalize ${ROLE_STYLES[u.role] ?? ''}`}>
                          {u.role.replace(/_/g, ' ')}
                        </span>
                      </td>
                      <td className="px-4 py-3">
