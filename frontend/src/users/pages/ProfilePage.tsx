...
// ── Inline form field helpers (same style as original) ────────────────────────
...
function FormSelect({ label, required, children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement> & { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">
        {label}{required && <span className="text-red-500"> *</span>}
    ...
    <div>
      ...
      <div className="relative">
        ...
        <span onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer">
          {show
            ? <EyeIcon className="fill-gray-400 w-4 h-4" />
            : <EyeCloseIcon className="fill-gray-400 w-4 h-4" />}
        </span>
      </div>
    </div>
  );
...
export default function ProfilePage() {
  ...
  const saveProfile = async (e: React.FormEvent) => {
    ...
    try {
      await client.patch(`/v1/users/${user!.user_id}`, profile);
      updateUser(profile);
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5">
        ...
        <div>
          ...
          <span className={`mt-1 inline-block px-2 py-0.5 text-xs font-medium rounded-full capitalize ${
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5">
        ...
        <form onSubmit={saveProfile} className="space-y-4">
          ...
          <FormSelect label="Gender" value={profile.gender ?? 'male'} onChange={e => setP('gender', e.target.value)}>
