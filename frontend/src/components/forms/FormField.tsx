...
const base = `w-full rounded-lg border border-gray-300 dark:border-gray-700
  bg-white dark:bg-gray-800 px-3 py-2.5 text-sm text-gray-800 dark:text-white
...
export function FormTextarea({ label, required, error, hint, wrapperClassName, ...props }: TextareaProps) {
  return (
    <FieldWrapper label={label} required={required} error={error} hint={hint} className={wrapperClassName}>
      <textarea rows={3} className={`${base} resize-none ${error ? 'border-red-400' : ''}`} {...props} />
    </FieldWrapper>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap">{title}</h4>
